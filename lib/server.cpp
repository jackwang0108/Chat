...
int ftp_fd[QUE_NUM];   // FTP套接字文件描述符队列
int voice_fd[QUE_NUM]; // 语音传输套接字文件描述符队列
...
/**
 * @brief 检查文件或目录是否存在, 不存在则创建文件
 * @param path 检查的文件
 * @return true 文件/目录已经存在或者已经创建; false 文件/目录不存在且创建失败
 */
bool checkPath(const fs::path &path, bool isDir)
{
    if (fs::exists(path))
        return true;
    else
 ...
 * @param cmd CMD结构体, 保存FTP命令和文件路径
 */
void ftpOfflineSend(int sock_fd, CMD cmd)
{
    sleep(1);
...
/**
 * @brief ftp在线传输文件
 * @param FTP_SEND 使用的FTP套接字文件描述符
 */
void ftpOnline(int FTP_SEND)
{
...
void *ftp_func(void *arg)
{
    ...
    if (cmd.method == 0)
    {
        ...
        ftpOfflineSend(ftp_fd[0], cmd);
    }
    else if (cmd.method == 1)
    {
        cout << "offline_recv function." << endl;
        ftpOfflineRecv(ftp_fd[0], cmd);
    }
...
void *mainWorker(void *arg)
{
    ...
    if (idVerify(conn_fd[info->NO]) == 0)
    {
        idVerify(conn_fd[info->NO]);
    }
